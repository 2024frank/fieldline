...
export type Device = { eui: string; name: string; typeId: string; typeName: string; status: string; lastSeenAt: string | null; battery: number | null; latest: Latest | null; gatewayEui?: string | null; rssi?: number | null; snr?: number | null; enabled?: boolean; config?: Record<string, unknown> };
export type Reading = { time: string; measurements: Record<string, number>; rssi: number; snr: number };
export type Alert = { id: string; type: "sensor_offline" | "gateway_offline" | "low_battery"; target: string; message: string; time: string };
...
export const api = {
  ...
  createDeviceType: (body: Omit<DeviceType, "id">) => request<DeviceType>("/device-types", { method: "POST", body: JSON.stringify(body) }),
